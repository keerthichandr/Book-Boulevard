...
books_df = pd.read_csv(r"goodreads_data.csv")
...
title_to_index = pd.Series(books_df.index, index=books_df['Book'].apply(lambda x: x.lower()))
...
def recommend_books(title, model=model, books_df=books_df, title_to_index=title_to_index):
    ...
    return books_df['Book'].iloc[indices].values.tolist()
